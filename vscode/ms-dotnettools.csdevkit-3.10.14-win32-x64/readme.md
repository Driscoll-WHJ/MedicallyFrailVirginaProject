...
## Write your C# code faster with AI-powered assistance

[GitHub Copilot](https://code.visualstudio.com/docs/copilot/overview) is an AI-powered coding assistant integrated into VS Code. It provides code suggestions, explanations, and automated implementations based on natural language prompts and existing code context. Copilot has been trained on public code repositories and can assist with your C# development. To get set up, follow the instructions here: [GitHub Copilot for Visual Studio Code](https://code.visualstudio.com/docs/copilot/setup).

## Collect tracing information

Use the `Record C#DK Trace` command from the Command Palette to capture runtime traces from active C# Dev Kit processes whenever you need deeper diagnostics. The command guides you through selecting processes, verifying `dotnet-trace`, and choosing an output location for the generated `.nettrace` files. For an in-depth walkthrough of the multi-process tracing experience and advanced configuration options, see `dotnet-trace-profiling.md`.
